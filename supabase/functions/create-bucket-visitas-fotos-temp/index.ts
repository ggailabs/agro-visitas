...
Deno.serve(async (req) => {
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for visitas-fotos" ON storage.objects FOR SELECT USING (bucket_id = 'visitas-fotos');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for visitas-fotos" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'visitas-fotos');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for visitas-fotos" ON storage.objects FOR UPDATE USING (bucket_id = 'visitas-fotos');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for visitas-fotos" ON storage.objects FOR DELETE USING (bucket_id = 'visitas-fotos');`
    ];
